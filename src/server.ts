...
const mcp = new Server(
  { name: SERVER_NAME, version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'Group messages include group_config_path and group_memory_path in the meta. On the FIRST message from a group in this session, Read group_config_path (config.md) for personality/goals/instructions/cron jobs. Follow those for all messages in that group. If the file is empty or missing, use your default personality. IMPORTANT for speed: read each group\'s config.md AT MOST ONCE per session — once you have a group\'s personality in context, do NOT re-read it for subsequent messages in that group.',
      '',
      'SPEED: When you receive a direct instruction to reply to a specific message (the message text and chat_id are given to you), reply to it directly with the reply tool. Do NOT call the unreplied tool, do NOT re-scan all chats, and do NOT re-read config files you already read this session. Go straight to answering. Only call unreplied when explicitly asked to catch up on everything (e.g. on session start).',
      '',
